...
void Graph_DG::createGraph(int kind) {
    ...
    while (count != this->edge) {
        ...
        //无向图添加上这行代码
        if (kind == 2)
            arc[end - 1][start - 1] = weight;//无向图为对称矩阵
        ++count;
    }
}
...
void Graph_DG::print() {
    ...
    int count_col = 0; //打印列的标签
                       //开始打印
    while (count_row != this->vexnum) {//两重循环嵌套输出邻接矩阵
        count_col = 0;
...
void Graph_DG::print_path() {
    ...
    int temp = 0;
    for (row = 0; row < this->vexnum; row++) {
        for (col = row + 1; col < this->vexnum; col++) {//将数值转化为字符串，返回对应的字符串。
            cout << "v" << to_string(row + 1) << "---" << "v" << to_string(col + 1) << " 权值: "
                << this->dis[row][col] << " 路径: " << " v" << to_string(row + 1);
            temp = path[row][col];
            //循环输出途径的每条路径。
            while (temp != col) {
